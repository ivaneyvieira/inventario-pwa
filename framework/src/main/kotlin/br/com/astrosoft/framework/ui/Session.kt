...
class SessionScoped<out R>(private val clazz: Class<R>) : ReadOnlyProperty<Any?, R> {
  ...
  private fun getOrCreate(): R =
    checkUIThread().session.getOrCreate()
...
object Session {
  ...
  val current: VaadinSession
    get() = VaadinSession.getCurrent()
            ?: throw IllegalStateException("Not in UI thread")
...
object Cookies {
  ...
  operator fun get(name: String): Cookie? =
    currentRequest.cookies?.firstOrNull { it.name == name }
...
fun checkUIThread() =
  UI.getCurrent()
  ?: throw IllegalStateException("Not in UI thread, or UI.init() is currently ongoing")