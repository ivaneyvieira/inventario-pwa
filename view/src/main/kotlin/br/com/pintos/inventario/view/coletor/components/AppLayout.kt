...
open class AppHeaderLayout: Component(), HasComponents, HasSize {
  init {
    setSizeFull()
    element.setAttribute("fullbleed", "")
  }
}
...
class NavMenuItem(icon: VaadinIcon, caption: String): Button(caption, Icon(icon)) {
  init {
    ...
    //TODO themes.add("large")
  }

  var selected: Boolean
    get() = classNames.contains("selected")
    set(value) {
      classNames.set("selected", value)
    }
}

fun (@VaadinDsl HasComponents).navMenuItem(icon: VaadinIcon, caption: String,
                                           block: (@VaadinDsl NavMenuItem).() -> Unit = {}) =
  init(NavMenuItem(icon, caption), block)
